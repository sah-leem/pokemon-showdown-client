(function (exports, $) {
	...
	exports.TeambuilderRoom = exports.Room.extend({
		...
		updateTeamInterface: function () {
			...
			if (this.exportMode) {
				if (this.curFolder) {
					buf = '<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> ' + I18n.t('List') + '</button></div>';
					...
					buf = '<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> ' + I18n.t('List') + '</button> <button name="saveBackup" class="savebutton button"><i class="fa fa-floppy-o"></i> ' + I18n.t('Save') + '</button></div>';
		...
		updateTeamList: function (resetScroll) {
			...
			if (!this.curFolder) {
				...
				if (this.curFolder.slice(-1) === '/') {
					...
					if (filterFolder) {
						buf += '<h2><i class="fa fa-folder-open"></i> ' + filterFolder + ' <button class="button small" style="margin-left:5px" name="renameFolder"><i class="fa fa-pencil"></i> ' + I18n.t('Rename') + '</button> <button class="button small" style="margin-left:5px" name="promptDeleteFolder"><i class="fa fa-times"></i> ' + I18n.t('Remove') + '</button></h2>';
					} else {
			...
			var newTeamButtonText = I18n.t('New Team');
			if (filterFolder) newTeamButtonText = I18n.t('New Team in folder');
			...
			buf += '<p><button name="newTop" value="team" class="button big"><i class="fa fa-plus-circle"></i> ' + newTeamButtonText + '</button> ' +
				'<button name="newTop" value="box" class="button big"><i class="fa fa-archive"></i> ' + I18n.t('New Box') + '</button> ' +
			...
			if (!teams.length) {
				if (this.deletedTeamLoc >= 0) {
					buf += '<li><button name="undoDelete"><i class="fa fa-undo"></i> ' + I18n.t('Undo Delete') + '</button></li>';
				}
				...
				for (var i = 0; i < teams.length + 1; i++) {
					...
					buf += '</small></div><button name="edit" value="' + i + '"><i class="fa fa-pencil" aria-label="Edit" title="Edit (you can also just click on the team)"></i></button><button name="duplicate" value="' + i + '" title="Duplicate" aria-label="Duplicate"><i class="fa fa-clone"></i></button><button name="delete" value="' + i + '"><i class="fa fa-trash"></i> ' + I18n.t('Delete') + '</button></li>';
			...
			buf += '</ul><p>';
			if (atLeastOne) {
				buf += '<button name="new" value="team" class="button"><i class="fa fa-plus-circle"></i> ' + newTeamButtonText + '</button> <button name="new" value="box" class="button"><i class="fa fa-archive"></i> ' + I18n.t('New Box') + '</button> ';
			}
			buf += '<button class="button" name="send" value="/teams">' + I18n.t('View teams uploaded to server') + '</button>';
			buf += '</p>';

			if (window.nodewebkit) {
				buf += '<button name="revealFolder" class="button"><i class="fa fa-folder-open"></i> ' + I18n.t('Reveal teams folder') + '</button> <button name="reloadTeamsFolder" class="button"><i class="fa fa-refresh"></i> ' + I18n.t('Reload teams files') + '</button> <button name="backup" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Backup/Restore all teams') + '</button>';
			} else if (this.curFolder) {
				buf += '<button name="backup" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Backup all teams from this folder') + '</button>';
			} else if (atLeastOne) {
				buf += '<p><strong>' + I18n.t('Clearing your cookies will delete your teams.') + '</strong> ';
				buf += '<span class="storage-warning">' + I18n.t('Browsers sometimes randomly clear cookies - you should upload your teams to the Showdown database') + ' ';
				buf += 'or make a backup yourself if you want to make sure you don\'t lose them.</span></p>';
				buf += '<button name="backup" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Backup/Restore all teams') + '</button>';
				buf += '<p>' + I18n.t('If you want to clear your cookies, you can use Backup/Restore to save your teams as text first.') + '</p>';
				var self = this;
				...
				buf += '<button name="backup" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Restore teams from backup') + '</button>';
			}
		...
		psExport: function () {
			...
			if (!team) return app.addPopupMessage(I18n.t('Add a Pokémon to your team before uploading it!'));
			buf.push(team);
		...
		updateTeamView: function () {
			...
			var buf = '';
			if (this.exportMode) {
				buf = '<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> ' + I18n.t('List') + '</button> <input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' + BattleLog.escapeHTML(this.curTeam.name) + '" /> <button name="saveImport" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Import/Export') + '</button> <button name="saveImport" class="savebutton button"><i class="fa fa-floppy-o"></i> ' + I18n.t('Save') + '</button></div>';
				...
				buf = '<div class="pad"><button name="back" class="button"><i class="fa fa-chevron-left"></i> ' + I18n.t('List') + '</button> ';
				buf += '<input class="textbox teamnameedit" type="text" class="teamnameedit" size="30" value="' + BattleLog.escapeHTML(this.curTeam.name) + '" /> ';
				buf += '<button name="import" class="button"><i class="fa fa-upload"></i> ' + I18n.t('Import/Export') + '</button> ';
				...
				if (this.loadingTeam) buf += '<div style="message-error">' + I18n.t('Downloading team from server...') + '</strong><br />';
				buf += '<label name="editMessage" style="display: none">';
				buf += '' + I18n.t('Remember to click the upload button below to sync your changes to the server!') + '</label><br />';
				if (exports.BattleFormats) {
					buf += '<li class="format-select">';
					buf += '<label class="label">Format:</label><button class="select formatselect teambuilderformatselect" name="format" value="' + this.curTeam.format + '">' + (isGenericFormat(this.curTeam.format) ? '<em>' + I18n.t('Select a format') + '</em>' : BattleLog.escapeFormat(this.curTeam.format)) + '</button>';
					buf += '<div id="typecoverage-panel"></div>';
					var btnClass = 'button' + (!this.curSetList.length || app.isDisconnected ? ' disabled' : '');
					buf += ' <button name="validate" class="' + btnClass + '"><i class="fa fa-check"></i> ' + I18n.t('Validate') + '</button> <button name="typeCoverage" class="' + btnClass + '">Type Chart</button></li>';
				}
				...
				for (i = 0; i < this.curSetList.length; i++) {
					if (this.curSetList.length < this.curTeam.capacity && this.deletedSet && i === this.deletedSetLoc) {
						buf += '<li><button name="undeleteSet" class="button"><i class="fa fa-undo"></i> ' + I18n.t('Undo Delete') + '</button></li>';
					}
				...
				if (i === 0) {
					buf += '<li><button name="import" class="button big"><i class="fa fa-upload"></i> ' + I18n.t('Import from text or URL') + '</button></li>';
				}
				if (i < this.curTeam.capacity) {
					buf += '<li><button name="addPokemon" class="button big"><i class="fa fa-plus"></i> ' + I18n.t('Add Pokémon') + '</button></li>';
				}
				buf += '</ol>';
			...
			/*	if (formatInfo && formatInfo !== true) {
				...
				buf += '<p><button name="psExport" type="submit" class="button exportbutton"> <i class="fa fa-upload"></i> ' + I18n.t('Upload to Showdown database (saves across devices)') + '</button>';
				...
				buf += '</p>';
				buf += '<p><button name="pokepasteExport" type="submit" class="button exportbutton"><i class="fa fa-upload"></i> ' + I18n.t('Upload to PokePaste') + '</button></p>';
				if (this.curTeam.format.includes('vgc')) {
					buf += '<p><button name="pokepasteExport" value="openteamsheet" type="submit" class="button exportbutton"><i class="fa fa-upload"></i> ' + I18n.t('Upload to PokePaste (Open Team Sheet)') + '</button></p>';
				}
				buf += '</form></div>';
			}
		...
		renderSet: function (set, i) {
			...
			if (!set.species) {
				if (this.deletedSet) {
					buf += '<div class="setmenu setmenu-left"><button name="undeleteSet" class="button"><i class="fa fa-undo"></i> ' + I18n.t('Undo Delete') + '</button></div>';
				}
				buf += '<div class="setmenu"><button name="importSet"><i class="fa fa-upload"></i>Import</button></div>';
				buf += '<div class="setchart" style="background-image:url(' + Dex.resourcePrefix + 'sprites/gen5/0.png);"><div class="setcol setcol-icon"><div class="setcell-sprite"></div><div class="setcell setcell-pokemon"><label>' + I18n.t('Pokémon') + '</label><input type="text" name="pokemon" class="textbox chartinput" value="" autocomplete="off" /></div></div></div>';
				buf += '</li>';
				return buf;
			}
			buf += '<div class="setmenu"><button name="copySet"><i class="fa fa-files-o"></i>' + I18n.t('Copy') + '</button> <button name="importSet"><i class="fa fa-upload"></i>' + I18n.t('Import/Export') + '</button> <button name="moveSet"><i class="fa fa-arrows"></i>' + I18n.t('Move') + '</button> <button name="deleteSet"><i class="fa fa-trash"></i>' + I18n.t('Delete') + '</button></div>';
			buf += '<div class="setchart-nickname">';
			buf += '<label>' + I18n.t('Nickname') + '</label><input type="text" name="nickname" class="textbox" value="' + BattleLog.escapeHTML(set.name || '') + '" placeholder="' + BattleLog.escapeHTML(species.baseSpecies) + '" />';
			buf += '</div>';
			...
			buf += '<div class="setcell setcell-pokemon"><label>' + I18n.t('Pokémon') + '</label><input type="text" name="pokemon" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en') ? I18n.pokemon(toID(set.species)) : set.species) + '" autocomplete="off" /></div></div>';

			// details
			buf += '<div class="setcol setcol-details"><div class="setrow">';
			buf += '<div class="setcell setcell-details"><label>' + I18n.t('Details') + '</label><button class="textbox setdetails" tabindex="-1" name="details">';
			...
			buf += '<span class="detailcell detailcell-first"><label>' + I18n.t('Level') + '</label>' + (set.level || 100) + '</span>';
			if (this.curTeam.gen > 1) {
				buf += '<span class="detailcell"><label>' + I18n.t('Gender') + '</label>' + GenderChart[set.gender || species.gender || 'N'] + '</span>';
				if (isLetsGo) {
					buf += '<span class="detailcell"><label>' + I18n.t('Happiness') + '</label>' + (typeof set.happiness === 'number' ? set.happiness : 70) + '</span>';
				} else if (this.curTeam.gen < 8 || isNatDex) {
					buf += '<span class="detailcell"><label>' + I18n.t('Happiness') + '</label>' + (typeof set.happiness === 'number' ? set.happiness : 255) + '</span>';
				}
				buf += '<span class="detailcell"><label>' + I18n.t('Shiny') + '</label>' + (set.shiny ? 'Yes' : 'No') + '</span>';
				if (!isLetsGo && this.curTeam.gen < 9) {
					if (this.curTeam.gen === 8 && !isNatDex) {
						if (isBDSP && species.baseSpecies === "Unown") {
							buf += '<span class="detailcell"><label>' + I18n.t('HP Type') + '</label>' + (set.hpType || 'Dark') + '</span>';
						}
				...
				if (this.curTeam.gen === 8 && !isBDSP) {
					if (!species.cannotDynamax && set.dynamaxLevel !== 10 && set.dynamaxLevel !== undefined) {
						buf += '<span class="detailcell"><label>' + I18n.t('Dmax Level') + '</label>' + (typeof set.dynamaxLevel === 'number' ? set.dynamaxLevel : 10) + '</span>';
					}
					if (species.canGigantamax || species.forme === 'Gmax') {
						buf += '<span class="detailcell"><label>' + I18n.t('Gmax') + '</label>' + (set.gigantamax || species.forme === 'Gmax' ? 'Yes' : 'No') + '</span>';
					}
				}
				if (this.curTeam.gen === 9) {
					buf += '<span class="detailcell"><label>' + I18n.t('Tera Type') + '</label>' + (set.teraType || species.requiredTeraType || species.types[0]) + '</span>';
				}
			}
			buf += '</button></div></div>';
			...
			if (this.curTeam.gen > 1 && !isLetsGo) buf += '<div class="setcell setcell-item"><label>' + I18n.t('Item') + '</label><input type="text" name="item" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.item) ? I18n.item(toID(set.item)) : set.item) + '" autocomplete="off" /></div>';
			if (this.curTeam.gen > 2 && !isLetsGo) buf += '<div class="setcell setcell-ability"><label>' + I18n.t('Ability') + '</label><input type="text" name="ability" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.ability) ? I18n.ability(toID(set.ability)) : set.ability) + '" autocomplete="off" /></div>';
			buf += '</div></div>';

			// moves
			if (!set.moves) set.moves = [];
			buf += '<div class="setcol setcol-moves"><div class="setcell"><label>' + I18n.t('Moves') + '</label>';
			buf += '<input type="text" name="move1" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.moves[0]) ? I18n.move(toID(set.moves[0])) : (set.moves[0] || '')) + '" autocomplete="off" /></div>';
			buf += '<div class="setcell"><input type="text" name="move2" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.moves[1]) ? I18n.move(toID(set.moves[1])) : (set.moves[1] || '')) + '" autocomplete="off" /></div>';
			buf += '<div class="setcell"><input type="text" name="move3" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.moves[2]) ? I18n.move(toID(set.moves[2])) : (set.moves[2] || '')) + '" autocomplete="off" /></div>';
			buf += '<div class="setcell"><input type="text" name="move4" class="textbox chartinput" value="' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en' && set.moves[3]) ? I18n.move(toID(set.moves[3])) : (set.moves[3] || '')) + '" autocomplete="off" /></div>';
			buf += '</div>';

			// stats
			buf += '<div class="setcol setcol-stats"><div class="setrow"><label>' + I18n.t('Stats') + '</label><button class="textbox setstats" name="stats">';
			buf += '<span class="statrow statrow-head"><label></label> <span class="statgraph"></span> <em>' + (!isLetsGo ? I18n.t('EV') : I18n.t('AV')) + '</em></span>';
			var stats = {};
			...
			for (var j in BattleStatNames) {
				...
				var statName = this.curTeam.gen === 1 && j === 'spa' ? 'Spc' : I18n.stat(BattleStatNames[j]);
		...
		typeCoverage: function () {
		        var $panel = this.$el.find('#typecoverage-panel');
		        if ($panel.find('table').length) { $panel.html(''); return; }
		        if (!this.curSetList || !this.curSetList.length) return;
		        
		        var GEN5_TYPES = ['Normal','Fire','Water','Electric','Grass','Ice','Fighting','Poison','Ground','Flying','Psychic','Bug','Rock','Ghost','Dragon','Dark','Steel'];
		        ...
		        for (var i = 0; i < this.curSetList.length; i++) {
		                ...
		                var species = this.curTeam.dex.species.get(set.species);
		                if (!species.exists) continue;
		                team.push({ name: species.name, types: species.types, icon: Dex.getPokemonIcon(species.name), ability: set.ability || '' });
		        }
		        ...
		        function getEff(atkType, defTypes, ability) {
                        ...
                        if (abilityId === 'sapsipper' && atkType === 'Grass') return 0;
		                for (var i = 0; i < defTypes.length; i++) {
		                        var typeData = Dex.types.get(defTypes[i]);
		                        if (!typeData || !typeData.damageTaken) continue;
		                        var val = typeData.damageTaken[atkType] || 0;
		                        if (val === 1) mult *= 2;
		        ...
		        buf += '<div style="padding:8px;border-bottom:1px solid rgba(128,128,128,.25)">';
		        buf += '<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:6px">';
		        buf += '<strong style="font-size:13px;font-family:Verdana,Geneva,sans-serif">Defensive Coverage</strong>';
		        buf += '<button name="typeCoverage" class="button" style="font-size:12px;padding:4px 12px"><i class="fa fa-times"></i> Close</button>';
		        buf += '</div>';
		        
		        // header row
		        buf += '<table id="tc-grid"><tr><th style="text-align:left;color:#fff;font-family:Verdana,Geneva,sans-serif">Type</th>';
		        for (var i = 0; i < team.length; i++) {
		                buf += '<th><span style="display:inline-block;width:40px;height:30px;' + team[i].icon + '" title="' + BattleLog.escapeHTML(team[i].name) + '"></span><br><span style="color:#fff;font-family:Verdana,Geneva,sans-serif;font-size:10px">' + BattleLog.escapeHTML(team[i].name) + '</span></th>';
		        }
		        buf += '<th style="color:#fff;font-family:Verdana,Geneva,sans-serif">Weak</th><th style="color:#fff;font-family:Verdana,Geneva,sans-serif">Resist</th></tr>';
		        
		        // type rows
		        for (var ti = 0; ti < GEN5_TYPES.length; ti++) {
		                var atkType = GEN5_TYPES[ti];
		                var totalWeak = 0, totalResist = 0;
		                buf += '<tr><td style="text-align:left;padding-left:4px;vertical-align:middle">' + Dex.getTypeIcon(atkType) + '</td>';
		                for (var pi = 0; pi < team.length; pi++) {
		                        var eff = getEff(atkType, team[pi].types, team[pi].ability);
		                        var cls = "tc-neutral";
		                        var label = "\u2014";
		                        if (eff >= 4) { cls = "tc-4x"; label = "4\u00d7"; totalWeak++; }
		                        else if (eff >= 2) { cls = "tc-2x"; label = "2\u00d7"; totalWeak++; }
		                        else if (eff === 0) { cls = "tc-immune"; label = "immune"; totalResist++; }
		                        else if (eff <= 0.25) { cls = "tc-quarter"; label = "\u00bc"; totalResist++; }
		                        else if (eff <= 0.5) { cls = "tc-half"; label = "\u00bd"; totalResist++; }
		                        buf += '<td class="' + cls + '">' + label + '</td>';
		                }
		                buf += '<td class="' + (totalWeak > 2 ? 'tc-weak' : totalWeak > 0 ? 'tc-weak' : 'tc-zero') + '">' + (totalWeak || '') + '</td>';
		                buf += '<td class="' + (totalResist > 0 ? 'tc-resist' : 'tc-zero') + '">' + (totalResist || '') + '</td>';
		                buf += '</tr>';
		        }
		        buf += '</table></div>';
		...
		clipboardInnerHTML: function () {
			...
			for (var i = 0; i < this.clipboardCount(); i++) {
				...
				buf += '<span class="species">' + BattleLog.escapeHTML((window.I18n && I18n.locale !== 'en') ? I18n.pokemon(toID(species.name)) : species.name) + '</span></div>';
		...
		importSetButtons: function () {
			...
			if (smogonFormatSets && smogonFormatSets['dex']) {
				...
				$smogonSetDiv.append(' <small>(<a target="_blank" href="' + this.smogdexLink(species) + '">StratDex&nbsp;analysis</a>)</small>');
			}
		...
		updateSetView: function () {
			...
			buf += '<div class="pokemonedit-buttons"><button name="closePokemonImport" class="button"><i class="fa fa-chevron-left"></i> Back</button> <button name="savePokemonImport" class="button"><i class="fa fa-floppy-o"></i> ' + I18n.t('Save') + '</button></div>';
		...
		renderTeambar: function () {
			...
			for (var i = start; i < end; i++) {
				...
				if (!set.species) {
					...
					buf += '<button disabled class="pokemon">' + pokemonicon + BattleLog.escapeHTML(set.name || ((window.I18n && I18n.locale !== 'en') ? I18n.pokemon(toID(set.species)) : this.curTeam.dex.species.get(set.species).baseSpecies) || '<i class="fa fa-plus"></i>') + '</button> ';
				} else {
					buf += '<button name="selectPokemon" value="' + i + '" class="pokemon">' + pokemonicon + BattleLog.escapeHTML(set.name || ((window.I18n && I18n.locale !== 'en') ? I18n.pokemon(toID(set.species)) : this.curTeam.dex.species.get(set.species).baseSpecies)) + '</button> ';
				}
			}
		...
		updateStatGraph: function () {
			...
			for (var stat in stats) {
				...
				if (color > 360) color = 360;
				var statName = this.curTeam.gen === 1 && stat === 'spa' ? 'Spc' : I18n.stat(BattleStatNames[stat]);
				buf += '<span class="statrow"><label>' + statName + '</label> <span class="statgraph"><span style="width:' + width + 'px;background:hsl(' + color + ',40%,75%);"></span></span> ' + evBuf + '</span>';
			}
			...
			if (this.curTeam.gen > 2 && supportsEVs) buf += '<div><em>' + I18n.t('Remaining:') + '</em></div>';
		...
		smogdexLink: function (s) {
		        var name = (s.name || s).toLowerCase().replace(/[^a-z0-9-]/g, '');
		...
		updateStatForm: function (setGuessed) {
			...
			buf += '<p class="suggested"><small>' + I18n.t('Guessed spread:') + '';
			if (role === '?') {
				buf += ' ' + I18n.t("(Please choose 4 moves to get a guessed spread)") + ' (<a target="_blank" href="' + this.smogdexLink(species) + '">StratDex&nbsp;analysis</a>)</small></p>';
			} else {
				buf += ' </small><button name="setStatFormGuesses" class="button">' + role + ': ';
				for (var i in BattleStatNames) {
					if (guessedEVs[i]) {
						var statName = this.curTeam.gen === 1 && i === 'spa' ? 'Spc' : I18n.stat(BattleStatNames[i]);
				...
				buf += '</button><small> (<a target="_blank" href="' + this.smogdexLink(species) + '">StratDex&nbsp;analysis</a>)</small></p>';
			...
			buf += '<div><label>' + I18n.t('HP') + '</label></div><div><label>' + I18n.t('Attack') + '</label></div><div><label>' + I18n.t('Defense') + '</label></div><div>';
			if (this.curTeam.gen === 1) {
				buf += '<label>Special</label></div>';
			} else {
				buf += '<label>' + I18n.t('Sp. Atk') + '</label></div><div><label>' + I18n.t('Sp. Def') + '</label></div>';
			}

			buf += '<div><label>' + I18n.t('Speed') + '</label></div></div>';

			buf += '<div class="col basestatscol"><div><em>' + I18n.t('Base') + '</em></div>';
			for (var i in stats) {
			...
			if (this.curTeam.gen > 2) {
				buf += '<div class="col ivcol"><div><strong>' + I18n.t('IVs') + '</strong></div>';
				if (!set.ivs) set.ivs = {};
				for (var i in stats) {
				...
				if (hpType && !this.canHyperTrain(set) && !(this.curTeam && this.curTeam.format && this.curTeam.format.includes('pokemmo'))) {
					...
					buf += '<div style="margin-left:-80px;text-align:right"><select name="ivspread" class="button">';
					buf += '<option value="" selected>' + I18n.t('IV spreads') + '</option>';
			...
			if (this.curTeam.gen > 2) {
				buf += '<p style="clear:both">' + I18n.t('Nature:') + ' <select name="nature" class="button">';
				for (var i in BattleNatures) {
					var curNature = BattleNatures[i];
					buf += '<option value="' + i + '"' + (curNature === nature ? 'selected="selected"' : '') + '>' + I18n.nature(i);
					if (curNature.plus) {
		...
		checkStatOptimizations: function () {
			...
			if (optimized) {
				...
				buf += '<small><em>' + I18n.t('Protip:') + '</em> ' + I18n.t('Use a different nature to') + ' ' + msg + ': </small>';
		...
		detailsChange: function (e) {
			...
			buf += '<span class="detailcell detailcell-first"><label>' + I18n.t('Level') + '</label>' + (set.level || 100) + '</span>';
			if (this.curTeam.gen > 1) {
				buf += '<span class="detailcell"><label>' + I18n.t('Gender') + '</label>' + GenderChart[set.gender || 'N'] + '</span>';
				if (isLetsGo) {
					buf += '<span class="detailcell"><label>' + I18n.t('Happiness') + '</label>70</span>';
				} else {
					if (this.curTeam.gen < 8 || isNatDex) buf += '<span class="detailcell"><label>' + I18n.t('Happiness') + '</label>' + (typeof set.happiness === 'number' ? set.happiness : 255) + '</span>';
				}
				buf += '<span class="detailcell"><label>' + I18n.t('Shiny') + '</label>' + (set.shiny ? 'Yes' : 'No') + '</span>';
				if (!isLetsGo && (this.curTeam.gen < 8 || isNatDex) && !(this.curTeam.format && this.curTeam.format.includes("pokemmo"))) buf += '<span class="detailcell"><label>' + I18n.t('HP Type') + '</label>' + (set.hpType || 'Dark') + '</span>';
				if (this.curTeam.gen === 8 && !isBDSP) {
					if (!species.cannotDynamax) {
						buf += '<span class="detailcell"><label>' + I18n.t('Dmax Level') + '</label>' + (typeof set.dynamaxLevel === 'number' ? set.dynamaxLevel : 10) + '</span>';
					}
		...
		chartSet: function (val, selectNext) {
			...
			var displayVal = val;
			if (window.I18n && I18n.locale !== 'en' && val) {
				if (inputName === 'pokemon') displayVal = I18n.pokemon(toID(val));
				else if (inputName === 'ability') displayVal = I18n.ability(toID(val));
				else if (inputName === 'item') displayVal = I18n.item(toID(val));
				else if (inputName.substr(0,4) === 'move') displayVal = I18n.move(toID(val));
			}
			input.val(displayVal).removeClass('incomplete');
			switch (inputName) {
			case 'pokemon':
		...
		speedTierData: {
		        'gen5pokemmoou': [
		                {s:'Ninjask',b:160,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Electrode',b:150,e:252,n:1.1,d:'Timid 252'},
		                {s:'Jolteon',b:130,e:252,n:1.1,d:'Timid 252'},
		                {s:'Crobat',b:130,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Weavile',b:125,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Alakazam',b:120,e:252,n:1.1,d:'Timid 252'},
		                {s:'Starmie',b:115,e:252,n:1.1,d:'Timid 252'},
		                {s:'Raikou',b:115,e:252,n:1.1,d:'Timid 252'},
		                {s:'Ambipom',b:115,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Serperior',b:113,e:252,n:1.1,d:'Timid 252'},
		                {s:'Scolipede',b:112,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Gengar',b:110,e:252,n:1.1,d:'Timid 252'},
		                {s:'Espeon',b:110,e:252,n:1.1,d:'Timid 252'},
		                {s:'Froslass',b:110,e:252,n:1.1,d:'Timid 252'},
		                {s:'Durant',b:109,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Terrakion',b:108,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Infernape',b:108,e:252,n:1.1,d:'Naive 252'},
		                {s:'Cobalion',b:108,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Virizion',b:108,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Zoroark',b:105,e:252,n:1.1,d:'Timid 252'},
		                {s:'Mienshao',b:105,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Mismagius',b:105,e:252,n:1.1,d:'Timid 252'},
		                {s:'Garchomp',b:102,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Staraptor',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Salamence',b:100,e:252,n:1.1,d:'Naive 252'},
		                {s:'Volcarona',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Jirachi',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Ninetales',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Flygon',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Hydreigon',b:98,e:252,n:1.1,d:'Timid 252'},
		                {s:'Haxorus',b:97,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Sigilyph',b:97,e:252,n:1.1,d:'Timid 252'},
		                {s:'Gliscor',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Darmanitan',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Arcanine',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Sharpedo',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Krookodile',b:92,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Lucario',b:90,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Roserade',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Porygon-Z',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Excadrill',b:88,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Rotom-Wash',b:86,e:252,n:1.1,d:'Timid 252'},
		                {s:'Rotom-Heat',b:86,e:252,n:1.1,d:'Timid 252'},
		                {s:'Nidoking',b:85,e:252,n:1.1,d:'Timid 252'},
		                {s:'Kingdra',b:85,e:252,n:1.1,d:'Timid 252'},
		                {s:'Heracross',b:85,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Toxicroak',b:85,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Gyarados',b:81,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Chandelure',b:80,e:252,n:1.1,d:'Timid 252'},
		                {s:'Dragonite',b:80,e:252,n:1.0,d:'Adamant 252'},
		                {s:'Mamoswine',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Venusaur',b:80,e:252,n:1.1,d:'Timid 252'},
		                {s:'Gallade',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Blaziken',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Breloom',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Metagross',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Cloyster',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Bisharp',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Crawdaunt',b:55,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Zapdos',b:100,e:0,n:1.0,d:'Bold 0'},
		                {s:'Tentacruel',b:100,e:0,n:1.0,d:'Bold 0'},
		                {s:'Suicune',b:85,e:0,n:1.0,d:'Bold 0'},
		                {s:'Cresselia',b:85,e:0,n:1.0,d:'Bold 0'},
		                {s:'Togekiss',b:80,e:0,n:1.0,d:'Calm 0'},
		                {s:'Heatran',b:77,e:0,n:1.0,d:'Calm 0'},
		                {s:'Skarmory',b:70,e:0,n:1.0,d:'Impish 0'},
		                {s:'Politoed',b:70,e:0,n:1.0,d:'Bold 0'},
		                {s:'Scizor',b:65,e:4,n:1.0,d:'Adamant 4'},
		                {s:'Pelipper',b:65,e:0,n:1.0,d:'Bold 0'},
		                {s:'Weezing',b:60,e:0,n:1.0,d:'Bold 0'},
		                {s:'Jellicent',b:60,e:0,n:1.0,d:'Bold 0'},
		                {s:'Magnezone',b:60,e:0,n:1.0,d:'Modest 0'},
		                {s:'Blissey',b:55,e:0,n:1.0,d:'Bold 0'},
		                {s:'Tyranitar',b:61,e:0,n:1.0,d:'Careful 0'},
		                {s:'Chansey',b:50,e:0,n:1.0,d:'Bold 0'},
		                {s:'Sableye',b:50,e:0,n:1.0,d:'Bold 0'},
		                {s:'Donphan',b:50,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Conkeldurr',b:45,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Forretress',b:40,e:0,n:1.0,d:'Relaxed 0'},
		                {s:'Bronzong',b:33,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Slowbro',b:30,e:0,n:1.0,d:'Bold 0'},
		                {s:'Slowking',b:30,e:0,n:1.0,d:'Calm 0'},
		                {s:'Reuniclus',b:30,e:0,n:0.9,d:'Quiet 0'},
		                {s:'Amoonguss',b:30,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Snorlax',b:30,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Ferrothorn',b:20,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmouu': [
		                {s:'Ninjask',b:160,e:252,n:1.1,d:'Jolly 252'},{s:'Crobat',b:130,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Ambipom',b:115,e:252,n:1.1,d:'Jolly 252'},{s:'Raikou',b:115,e:252,n:1.1,d:'Timid 252'},
		                {s:'Scolipede',b:112,e:252,n:1.1,d:'Jolly 252'},{s:'Espeon',b:110,e:252,n:1.1,d:'Timid 252'},
		                {s:'Froslass',b:110,e:252,n:1.1,d:'Timid 252'},{s:'Virizion',b:108,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Cobalion',b:108,e:252,n:1.1,d:'Jolly 252'},{s:'Flygon',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Ninetales',b:100,e:252,n:1.1,d:'Timid 252'},{s:'Entei',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Shaymin',b:100,e:252,n:1.1,d:'Timid 252'},{s:'Tentacruel',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Haxorus',b:97,e:252,n:1.1,d:'Jolly 252'},{s:'Sigilyph',b:97,e:252,n:1.1,d:'Timid 252'},
		                {s:'Yanmega',b:95,e:252,n:1.1,d:'Timid 252'},{s:'Darmanitan',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Krookodile',b:92,e:252,n:1.1,d:'Jolly 252'},{s:'Porygon-Z',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Meloetta',b:90,e:252,n:1.1,d:'Timid 252'},{s:'Roserade',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Rotom-Heat',b:86,e:252,n:1.1,d:'Timid 252'},{s:'Rotom-Mow',b:86,e:252,n:1.1,d:'Timid 252'},
		                {s:'Nidoking',b:85,e:252,n:1.1,d:'Timid 252'},{s:'Heracross',b:85,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Gligar',b:85,e:252,n:1.1,d:'Jolly 252'},{s:'Kingdra',b:85,e:252,n:1.1,d:'Timid 252'},
		                {s:'Chandelure',b:80,e:252,n:1.1,d:'Timid 252'},{s:'Medicham',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Venusaur',b:80,e:252,n:1.1,d:'Timid 252'},{s:'Feraligatr',b:78,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Honchkrow',b:71,e:252,n:1.1,d:'Jolly 252'},{s:'Bisharp',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Cloyster',b:70,e:252,n:1.1,d:'Jolly 252'},{s:'Politoed',b:70,e:0,n:1.0,d:'Bold 0'},
		                {s:'Empoleon',b:60,e:0,n:1.0,d:'Calm 0'},{s:'Magnezone',b:60,e:0,n:1.0,d:'Modest 0'},
		                {s:'Porygon2',b:60,e:0,n:1.0,d:'Bold 0'},{s:'Machamp',b:55,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Blissey',b:55,e:0,n:1.0,d:'Bold 0'},{s:'Donphan',b:50,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Tangrowth',b:50,e:0,n:1.0,d:'Bold 0'},{s:'Druddigon',b:48,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Hippowdon',b:47,e:0,n:1.0,d:'Impish 0'},{s:'Rhyperior',b:40,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Forretress',b:40,e:0,n:1.0,d:'Relaxed 0'},{s:'Gastrodon',b:39,e:0,n:1.0,d:'Bold 0'},
		                {s:'Quagsire',b:35,e:0,n:1.0,d:'Bold 0'},{s:'Spiritomb',b:35,e:0,n:1.0,d:'Calm 0'},
		                {s:'Bronzong',b:33,e:0,n:0.9,d:'Relaxed 0'},{s:'Slowking',b:30,e:0,n:1.0,d:'Calm 0'},
		                {s:'Snorlax',b:30,e:0,n:1.0,d:'Adamant 0'},{s:'Cofagrigus',b:30,e:0,n:1.0,d:'Bold 0'},
		                {s:'Nidoqueen',b:76,e:0,n:1.0,d:'Bold 0'},{s:'Umbreon',b:65,e:0,n:1.0,d:'Calm 0'},
		                {s:'Gigalith',b:25,e:0,n:1.0,d:'Adamant 0'},
		        ],
		        'gen5pokemmonu': [
		                {s:'Electrode',b:150,e:252,n:1.1,d:'Timid 252'},{s:'Jolteon',b:130,e:252,n:1.1,d:'Timid 252'},
		                {s:'Sceptile',b:120,e:252,n:1.1,d:'Timid 252'},{s:'Alakazam',b:120,e:252,n:1.1,d:'Timid 252'},
		                {s:'Cinccino',b:115,e:252,n:1.1,d:'Jolly 252'},{s:'Durant',b:109,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Zoroark',b:105,e:252,n:1.1,d:'Timid 252'},{s:'Mismagius',b:105,e:252,n:1.1,d:'Timid 252'},
		                {s:'Moltres',b:90,e:252,n:1.1,d:'Timid 252'},{s:'Venomoth',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Roserade',b:90,e:252,n:1.1,d:'Timid 252'},{s:'Xatu',b:95,e:252,n:1.1,d:'Timid 252'},
		                {s:'Arcanine',b:95,e:252,n:1.1,d:'Jolly 252'},{s:'Sharpedo',b:95,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Rotom',b:91,e:252,n:1.1,d:'Timid 252'},{s:'Golbat',b:90,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Toxicroak',b:85,e:252,n:1.1,d:'Jolly 252'},{s:'Qwilfish',b:85,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Golduck',b:85,e:252,n:1.1,d:'Timid 252'},{s:'Skuntank',b:84,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Servine',b:83,e:252,n:1.1,d:'Timid 252'},{s:'Braviary',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Blaziken',b:80,e:252,n:1.1,d:'Jolly 252'},{s:'Gallade',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Gardevoir',b:80,e:252,n:1.1,d:'Timid 252'},{s:'Kabutops',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Altaria',b:80,e:0,n:1.0,d:'Calm 0'},{s:'Mandibuzz',b:80,e:0,n:1.0,d:'Bold 0'},
		                {s:'Blastoise',b:78,e:0,n:1.0,d:'Bold 0'},{s:'Hitmonchan',b:76,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Hitmontop',b:70,e:0,n:1.0,d:'Impish 0'},{s:'Ludicolo',b:70,e:252,n:1.1,d:'Timid 252'},
		                {s:'Poliwrath',b:70,e:0,n:1.0,d:'Bold 0'},{s:'Magneton',b:70,e:252,n:1.1,d:'Timid 252'},
		                {s:'Mantine',b:70,e:0,n:1.0,d:'Calm 0'},{s:'Lanturn',b:67,e:0,n:1.0,d:'Bold 0'},
		                {s:'Vaporeon',b:65,e:0,n:1.0,d:'Bold 0'},{s:'Clefable',b:60,e:0,n:1.0,d:'Bold 0'},
		                {s:'Abomasnow',b:60,e:0,n:1.0,d:'Quiet 0'},{s:'Swampert',b:60,e:0,n:1.0,d:'Relaxed 0'},
		                {s:'Torterra',b:56,e:0,n:1.0,d:'Adamant 0'},{s:'Hariyama',b:50,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Eelektross',b:50,e:0,n:1.0,d:'Modest 0'},{s:'Piloswine',b:50,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Golem',b:45,e:0,n:1.0,d:'Adamant 0'},{s:'Steelix',b:30,e:0,n:1.0,d:'Impish 0'},
		                {s:'Dusclops',b:25,e:0,n:1.0,d:'Bold 0'},{s:'Bronzor',b:23,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Torkoal',b:20,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmolcou': [
		                {s:'Sneasel',b:115,e:236,n:1.1,d:'Jolly 236'},{s:'Scyther',b:105,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Diglett',b:95,e:236,n:1.1,d:'Jolly 236'},{s:'Yanma',b:95,e:236,n:1.1,d:'Timid 236'},
		                {s:'Murkrow',b:91,e:236,n:1.1,d:'Jolly 236'},{s:'Ponyta',b:90,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Abra',b:90,e:236,n:1.1,d:'Timid 236'},{s:'Taillow',b:85,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Staryu',b:85,e:236,n:1.1,d:'Timid 236'},{s:'Aipom',b:85,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Gligar',b:85,e:236,n:1.1,d:'Jolly 236'},{s:'Misdreavus',b:85,e:236,n:1.1,d:'Timid 236'},
		                {s:'Magby',b:83,e:236,n:1.1,d:'Timid 236'},{s:'Gastly',b:80,e:236,n:1.1,d:'Timid 236'},
		                {s:'Stunky',b:74,e:236,n:1.1,d:'Jolly 236'},{s:'Drifloon',b:70,e:236,n:1.1,d:'Timid 236'},
		                {s:'Natu',b:70,e:236,n:1.1,d:'Timid 236'},{s:'Onix',b:70,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Drilbur',b:68,e:236,n:1.1,d:'Jolly 236'},{s:'Chinchou',b:67,e:236,n:1.1,d:'Timid 236'},
		                {s:'Carvanha',b:65,e:236,n:1.1,d:'Jolly 236'},{s:'Mienfoo',b:65,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Snivy',b:63,e:236,n:1.1,d:'Timid 236'},{s:'Rufflet',b:60,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Pawniard',b:60,e:236,n:1.1,d:'Jolly 236'},{s:'Meditite',b:60,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Larvesta',b:60,e:0,n:1.0,d:'Bold 0'},{s:'Vullaby',b:60,e:0,n:1.0,d:'Bold 0'},
		                {s:'Tangela',b:60,e:0,n:1.0,d:'Bold 0'},{s:'Scraggy',b:48,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Frillish',b:40,e:0,n:1.0,d:'Bold 0'},{s:'Porygon',b:40,e:0,n:1.0,d:'Bold 0'},
		                {s:'Koffing',b:35,e:0,n:1.0,d:'Bold 0'},{s:'Clamperl',b:32,e:0,n:1.0,d:'Modest 0'},
		                {s:'Bronzor',b:23,e:0,n:0.9,d:'Relaxed 0'},{s:'Foongus',b:15,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Trapinch',b:10,e:236,n:1.1,d:'Jolly 236'},{s:'Ferroseed',b:10,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmolcubers': [
		                {s:'Sneasel',b:115,e:236,n:1.1,d:'Jolly 236'},{s:'Scyther',b:105,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Diglett',b:95,e:236,n:1.1,d:'Jolly 236'},{s:'Yanma',b:95,e:236,n:1.1,d:'Timid 236'},
		                {s:'Murkrow',b:91,e:236,n:1.1,d:'Jolly 236'},{s:'Gligar',b:85,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Misdreavus',b:85,e:236,n:1.1,d:'Timid 236'},{s:'Meditite',b:60,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Tangela',b:60,e:0,n:1.0,d:'Bold 0'},{s:'Scraggy',b:48,e:236,n:1.1,d:'Jolly 236'},
		        ],
		        'gen5pokemmolcuu': [
		                {s:'Voltorb',b:100,e:236,n:1.1,d:'Timid 236'},{s:'Elekid',b:95,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Doduo',b:75,e:236,n:1.1,d:'Jolly 236'},{s:'Anorith',b:75,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Archen',b:70,e:236,n:1.1,d:'Jolly 236'},{s:'Treecko',b:70,e:236,n:1.1,d:'Timid 236'},
		                {s:'Mankey',b:70,e:236,n:1.1,d:'Jolly 236'},{s:'Cottonee',b:66,e:0,n:1.0,d:'Bold 0'},
		                {s:'Houndour',b:65,e:236,n:1.1,d:'Timid 236'},{s:'Joltik',b:65,e:236,n:1.1,d:'Timid 236'},
		                {s:'Zorua',b:65,e:236,n:1.1,d:'Timid 236'},{s:'Chimchar',b:61,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Growlithe',b:60,e:236,n:1.1,d:'Jolly 236'},{s:'Cranidos',b:58,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Axew',b:57,e:236,n:1.1,d:'Jolly 236'},{s:'Eevee',b:55,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Dwebble',b:55,e:236,n:1.1,d:'Jolly 236'},{s:'Kabuto',b:55,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Croagunk',b:50,e:236,n:1.1,d:'Jolly 236'},{s:'Darumaka',b:50,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Dratini',b:50,e:236,n:1.1,d:'Jolly 236'},{s:'Bagon',b:50,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Bulbasaur',b:45,e:236,n:1.1,d:'Timid 236'},{s:'Magnemite',b:45,e:0,n:1.0,d:'Modest 0'},
		                {s:'Squirtle',b:43,e:0,n:1.0,d:'Bold 0'},{s:'Totodile',b:43,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Larvitar',b:41,e:236,n:1.1,d:'Jolly 236'},{s:'Riolu',b:40,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Sandshrew',b:40,e:236,n:1.1,d:'Jolly 236'},{s:'Shellder',b:40,e:236,n:1.1,d:'Jolly 236'},
		                {s:'Snover',b:40,e:0,n:0.9,d:'Quiet 0'},{s:'Piplup',b:40,e:0,n:1.0,d:'Bold 0'},
		                {s:'Machop',b:35,e:0,n:1.0,d:'Adamant 0'},{s:'Timburr',b:35,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Shroomish',b:35,e:236,n:1.1,d:'Jolly 236'},{s:'Hippopotas',b:32,e:0,n:1.0,d:'Impish 0'},
		                {s:'Litwick',b:20,e:236,n:1.1,d:'Timid 236'},{s:'Slowpoke',b:15,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmovgcubers': [
		                {s:'Mewtwo',b:130,e:252,n:1.1,d:'Timid 252'},{s:'Jolteon',b:130,e:252,n:1.1,d:'Timid 252'},
		                {s:'Weavile',b:125,e:252,n:1.1,d:'Jolly 252'},{s:'Arceus',b:120,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Whimsicott',b:116,e:0,n:1.0,d:'Bold 0'},{s:'Raichu',b:110,e:252,n:1.1,d:'Timid 252'},
		                {s:'Terrakion',b:108,e:252,n:1.1,d:'Jolly 252'},{s:'Electabuzz',b:105,e:252,n:1.1,d:'Timid 252'},
		                {s:'Garchomp',b:102,e:252,n:1.1,d:'Jolly 252'},{s:'Palkia',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Salamence',b:100,e:252,n:1.1,d:'Naive 252'},{s:'Volcarona',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Rayquaza',b:95,e:252,n:1.1,d:'Jolly 252'},{s:'Kyogre',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Zekrom',b:90,e:252,n:1.1,d:'Jolly 252'},{s:'Reshiram',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Excadrill',b:88,e:252,n:1.1,d:'Jolly 252'},{s:'Kingdra',b:85,e:252,n:1.1,d:'Timid 252'},
		                {s:'Lugia',b:110,e:0,n:1.0,d:'Bold 0'},{s:'Cresselia',b:85,e:0,n:1.0,d:'Bold 0'},
		                {s:'Ho-Oh',b:90,e:0,n:1.0,d:'Adamant 0'},{s:'Dialga',b:90,e:0,n:1.0,d:'Modest 0'},
		                {s:'Groudon',b:90,e:0,n:1.0,d:'Adamant 0'},{s:'Giratina-Origin',b:90,e:0,n:1.0,d:'Modest 0'},
		                {s:'Heatran',b:77,e:0,n:1.0,d:'Calm 0'},{s:'Scizor',b:65,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Conkeldurr',b:45,e:0,n:1.0,d:'Adamant 0'},{s:'Amoonguss',b:30,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Ferrothorn',b:20,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmovgc2': [
		                {s:'Crobat',b:130,e:252,n:1.1,d:'Jolly 252'},{s:'Weavile',b:125,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Whimsicott',b:116,e:0,n:1.0,d:'Bold 0'},{s:'Serperior',b:113,e:252,n:1.1,d:'Timid 252'},
		                {s:'Raichu',b:110,e:252,n:1.1,d:'Timid 252'},{s:'Terrakion',b:108,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Mienshao',b:105,e:252,n:1.1,d:'Jolly 252'},{s:'Electabuzz',b:105,e:252,n:1.1,d:'Timid 252'},
		                {s:'Garchomp',b:102,e:252,n:1.1,d:'Jolly 252'},{s:'Zapdos',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Jirachi',b:100,e:252,n:1.1,d:'Jolly 252'},{s:'Salamence',b:100,e:252,n:1.1,d:'Naive 252'},
		                {s:'Volcarona',b:100,e:252,n:1.1,d:'Timid 252'},{s:'Slaking',b:100,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Hydreigon',b:98,e:252,n:1.1,d:'Timid 252'},{s:'Lilligant',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Excadrill',b:88,e:252,n:1.1,d:'Jolly 252'},{s:'Rotom-Wash',b:86,e:252,n:1.1,d:'Timid 252'},
		                {s:'Kingdra',b:85,e:252,n:1.1,d:'Timid 252'},{s:'Gyarados',b:81,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Milotic',b:81,e:0,n:1.0,d:'Bold 0'},{s:'Chandelure',b:80,e:252,n:1.1,d:'Timid 252'},
		                {s:'Dragonite',b:80,e:252,n:1.0,d:'Adamant 252'},{s:'Mamoswine',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Venusaur',b:80,e:252,n:1.1,d:'Timid 252'},{s:'Gallade',b:80,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Togekiss',b:80,e:0,n:1.0,d:'Calm 0'},{s:'Blastoise',b:78,e:0,n:1.0,d:'Bold 0'},
		                {s:'Heatran',b:77,e:0,n:1.0,d:'Calm 0'},{s:'Breloom',b:70,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Metagross',b:70,e:252,n:1.1,d:'Jolly 252'},{s:'Ludicolo',b:70,e:252,n:1.1,d:'Timid 252'},
		                {s:'Hitmontop',b:70,e:0,n:1.0,d:'Impish 0'},{s:'Politoed',b:70,e:0,n:1.0,d:'Bold 0'},
		                {s:'Scizor',b:65,e:0,n:1.0,d:'Adamant 0'},{s:'Pelipper',b:65,e:0,n:1.0,d:'Bold 0'},
		                {s:'Cresselia',b:85,e:0,n:1.0,d:'Bold 0'},{s:'Tyranitar',b:61,e:0,n:1.0,d:'Brave 0'},
		                {s:'Porygon2',b:60,e:0,n:1.0,d:'Bold 0'},{s:'Hariyama',b:50,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Sableye',b:50,e:0,n:1.0,d:'Bold 0'},{s:'Chansey',b:50,e:0,n:1.0,d:'Bold 0'},
		                {s:'Conkeldurr',b:45,e:0,n:1.0,d:'Adamant 0'},{s:'Rhyperior',b:40,e:0,n:1.0,d:'Adamant 0'},
		                {s:'Shedinja',b:40,e:0,n:1.0,d:'Adamant 0'},{s:'Gastrodon',b:39,e:0,n:1.0,d:'Bold 0'},
		                {s:'Bronzong',b:33,e:0,n:0.9,d:'Relaxed 0'},{s:'Reuniclus',b:30,e:0,n:0.9,d:'Quiet 0'},
		                {s:'Musharna',b:29,e:0,n:0.9,d:'Quiet 0'},{s:'Amoonguss',b:30,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Dusclops',b:25,e:0,n:1.0,d:'Bold 0'},{s:'Torkoal',b:20,e:0,n:0.9,d:'Relaxed 0'},
		                {s:'Ferrothorn',b:20,e:0,n:0.9,d:'Relaxed 0'},{s:'Shuckle',b:5,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		        'gen5pokemmoubers': [
		                {s:'Mewtwo',b:130,e:252,n:1.1,d:'Timid 252'},{s:'Jolteon',b:130,e:252,n:1.1,d:'Timid 252'},
		                {s:'Arceus',b:120,e:252,n:1.1,d:'Jolly 252'},{s:'Lugia',b:110,e:0,n:1.0,d:'Bold 0'},
		                {s:'Garchomp',b:102,e:252,n:1.1,d:'Jolly 252'},{s:'Palkia',b:100,e:252,n:1.1,d:'Timid 252'},
		                {s:'Rayquaza',b:95,e:252,n:1.1,d:'Jolly 252'},{s:'Zekrom',b:90,e:252,n:1.1,d:'Jolly 252'},
		                {s:'Reshiram',b:90,e:252,n:1.1,d:'Timid 252'},{s:'Kyogre',b:90,e:252,n:1.1,d:'Timid 252'},
		                {s:'Ho-Oh',b:90,e:0,n:1.0,d:'Adamant 0'},{s:'Dialga',b:90,e:0,n:1.0,d:'Modest 0'},
		                {s:'Groudon',b:90,e:0,n:1.0,d:'Adamant 0'},{s:'Giratina-Origin',b:90,e:0,n:1.0,d:'Modest 0'},
		                {s:'Ferrothorn',b:20,e:0,n:0.9,d:'Relaxed 0'},
		        ],
		...
		calcSpeedStat: function (base, ev, iv, level, natureMod) {
		        ...
		        if (natureMod === undefined) natureMod = 1.0;
		        return Math.floor((Math.floor((2 * base + iv + Math.floor(ev / 4)) * level / 100) + 5) * natureMod);
		},

		getFormatSpeedKey: function () {
		        var fmt = this.curTeam.format || '';
		        if (this.speedTierData[fmt]) return fmt;
		        if (fmt.includes('pokemmolcubers') || fmt.includes('pokemmolcuber')) return 'gen5pokemmolcubers';
		        if (fmt.includes('pokemmolcuu')) return 'gen5pokemmolcuu';
		        ...
		        if (fmt.includes('pokemmo')) return 'gen5pokemmoou';
		        return 'gen5pokemmoou';
		},

		getFormatLabel: function (key) {
		        if (key.includes('lcubers')) return 'LC Ubers';
		        if (key.includes('lcuu')) return 'LC UU';
		        if (key.includes('lcou') || key.includes('lc')) return 'LC OU';
		        if (key.includes('vgcubers')) return 'VGC Ubers';
		        if (key.includes('vgc')) return 'VGC';
		...
		updateSpeedTiers: function () {
		        ...
		        var set = this.curSet;
		        if (!set || !set.species) { $container.html(''); return; }
		        var species = this.curTeam.dex.species.get(set.species);
		        if (!species.exists) { $container.html(''); return; }
		        ...
		        for (var i = 0; i < entries.length; i++) {
		                ...
		                buf += '<span style="width:34px;text-align:right;font-weight:bold;font-size:12px;color:' + (e.self ? '#4a90d9' : 'inherit') + '">' + e.speed + '</span>';
		                buf += '<span style="width:80px;text-align:right;opacity:0.55;font-size:9px">' + BattleLog.escapeHTML(e.detail) + '</span>';
		                buf += '</div>';
		        }
		        buf += '</div></div>';
		        $container.html(buf);
